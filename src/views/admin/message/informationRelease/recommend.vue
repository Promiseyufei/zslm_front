						...
						<div class="operateUpfilesRight2">
							<template>
								...
								<div v-if="this.setSwitch == 2">
									...
									<!-- 表格 -->
									<messageTable :tableData3="tableData" :isSelect="0" :listTable="listTable" @setInfoRelation="setInfoState"
									 @del="delAdvise"></messageTable>
								</div>
							</template>
						</div>
					</div>
...
<script>
	export default {
		...
		methods: {
			...
			//删除主办院校
			deleteSchool: function(id, index) {
				// this.confirm(() => {
				//     this.post('/')
				// }, () => {
				//     this.message(true, '取消设置', 'info');sc
				// }, '确定取消当前相关院校专业?');
				var that = this
				this.confirm(() => {
					this.post('/admin/information/delappoinrelevantmajor', {
						infoId: that.infoId,
						majorId: id
					}).then((response) => {
						if (response.code == 0) {
							that.message(true, response.msg, 'success');
							that.shoolCount.splice(index, 1);
						} else {
