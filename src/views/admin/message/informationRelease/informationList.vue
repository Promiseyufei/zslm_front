<template>
	<div class="informationList">
		...
		<div class="information-table">
			<el-table @selection-change="handleSelectionChange" :header-cell-style="{background:'#f9fafc'}" :data="informationListtTable"
			 @current-change="handleCurrentChange" border style="width: 100%">
				<el-table-column type="selection" width="60"></el-table-column>
				<el-table-column label="编号" prop="id" width="100"></el-table-column>
				<el-table-column label="展示权重" width="80">
					<template slot-scope="scope">
						<el-input v-model="informationListtTable[scope.$index].weight" @focus="focusMajorWeigthCount(informationListtTable[scope.$index].weight)"
						 v-on:blur="changeMajorWeight(informationListtTable[scope.$index].id, informationListtTable[scope.$index].weight, scope.$index)"></el-input>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="120">
					<template slot-scope="scope">
						<div class="majorlist-icon">
							<i class="el-icon-search" @click="infoPage(informationListtTable[scope.$index].id)"></i>
	...
	export default {
		...
		methods: {
			...
			deleteInfos() {
				...
				this.confirm(() => {
					this.post('/admin/information/delinfos', {
						ids: selectId
					}).then(res => {
						if (res.code == 0) {
							that.gettableInfo();
...
<style>
	...
	.information-table .el-table .cell,
	.el-table th div,
...
<style scoped>
	...
	.informationList-query p,
	.informationList-contentlist p {
		font-size: 16px;
		color: #666;
		font-weight: bold;
	}

	.informationList-query i,
	.informationList-contentlist i {
		margin: 0 10px 0 20px;
	}

	.informationList-query,
	.informationList-contentlist {
		position: relative;
		display: flex;
		align-items: center;
		width: 1500px;
