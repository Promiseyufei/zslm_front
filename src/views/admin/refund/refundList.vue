<template>
	<div>
			...
			<el-table :data="tableData" border style="width: 100%" :header-cell-style="getRowClass">
				<el-table-column label="操作">
					<template slot-scope="scope">
						<el-button @click="jumpPage(scope.row.id)" type="text" icon="el-icon-search">
						</el-button>
						<el-button @click="handleClick(scope.row)" type="text" icon="el-icon-edit-outline">
						</el-button>
					</template>
				</el-table-column>
		...
		<!-- 分页 -->
		<div class="footer">
			<Page ref="page" :total="total" @pageChange="pageChange"></Page>
		</div>
		...
		methods: {
			...
			handleClick(val) {
				if (val.process_status == '进行中') {
					...
					if (val.approve_status == '通过') {
						this.type2 = 1
					} else if (val.approve_status == '驳回') {
						this.type2 = 2
					} else {
						this.type2 = 0
					}

					this.status = val.process_status == '进行中' ? 0 : 1
				} else {
					this.formdis = true
